...
class EmailController (
    ...
    fun sendTestDdayEmail(@PathVariable itemId: Long): ResponseEntity<MutableMap<String, Any>> {
        ...
        return runCatching {
            ...
            // D-Day 알림 이메일 발송
            val sentToEmail = emailService.sendDDayNotification(user.email, item)
            ...
            response["itemName"] = item.name ?: "" // 이 부분은 item 도메인 Kotlin 전환 이후 `item.name` 형태로 정리 예정
            response["userId"] = user.id
            response["userLoginId"] = user.loginId

            ResponseEntity.ok<MutableMap<String, Any>>(response)
        }.getOrElse { e ->
            // 예외 발생시 스택 트레이스 출력
            e.printStackTrace()
            response["success"] = false
            response["message"] = "이메일 발송 실패: " + e.message
            ResponseEntity.badRequest().body<MutableMap<String, Any>>(response)
        }
    }
}
